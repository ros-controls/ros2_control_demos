...
int main(int argc, char **argv) {
  ...
  auto pub = node->create_publisher<trajectory_msgs::msg::JointTrajectory>("/r6bot_controller/joint_trajectory", 10);
  ...
  for (int i = 0; i < trajectory_len; i++) {
    ...
    std::memcpy(trajectory_point_msg.positions.data(), joint_positions.data.data(),
                trajectory_point_msg.positions.size() * sizeof(double));
    std::memcpy(trajectory_point_msg.velocities.data(), joint_positions.data.data(),
    ...
    trajectory_point_msg.time_from_start.nanosec = 1E9/loop_rate * (double)(i - loop_rate*(i/loop_rate));
